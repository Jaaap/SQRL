...
function getPostData(href)
{
	if (typeof href == "string" && href.startsWith("sqrl://"))
	{
		...
		//FIXME: clear or zero-out all ArrayBuffers
		return ["client=" + encodeURIComponent(client), "server=" + encodeURIComponent(server), "ids=" + encodeURIComponent(sodium.crypto_sign_detached(client + server, SitePrivateKey, 'base64'))].join('&');
	}
	else
