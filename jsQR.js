...
function scan(matrix) {
	var loc = locate(matrix);
	if (!loc) {
		return null;
	}
	var extracted = extract(matrix, loc);
	var decoded = decode5(extracted.matrix);
	if (!decoded) {
		return null;
	}
	return {
		...
		loc: {
			topRightCorner: extracted.mappingFunction(loc.dimension, 0),
			topLeftCorner: extracted.mappingFunction(0, 0),
			bottomRightCorner: extracted.mappingFunction(loc.dimension, loc.dimension),
			bottomLeftCorner: extracted.mappingFunction(0, loc.dimension),
			topRightFinderPattern: loc.topRight,
			topLeftFinderPattern: loc.topLeft,
			bottomLeftFinderPattern: loc.bottomLeft,
			bottomRightAlignmentPattern: loc.alignmentPattern,
		},
	};
}
var defaultOptions = {
...
window.jsQR = function(data, width, height, providedOptions) {
...
function decodeMatrix(matrix) {
	...
	for (var _i = 0, dataBlocks_3 = dataBlocks; _i < dataBlocks_3.length; _i++) {
		...
		var correctedBytes = decode9(dataBlock.codewords, dataBlock.codewords.length - dataBlock.numDataCodewords);
		if (!correctedBytes) {
			return null;
		}
	...
	try {
		return decode6(resultBytes, version.versionNumber);
	}
	catch (_a) {
		return null;
	}
}
function decode5(matrix) {
	if (matrix == null) {
		return null;
	}
...
function extract(image, loc) {
	var qToS = quadrilateralToSquare({ x: 3.5, y: 3.5 }, { x: loc.dimension - 3.5, y: 3.5 }, { x: loc.dimension - 6.5, y: loc.dimension - 6.5 }, { x: 3.5, y: loc.dimension - 3.5 });
	var sToQ = squareToQuadrilateral(loc.topLeft, loc.topRight, loc.alignmentPattern, loc.bottomLeft);
	var transform = times(sToQ, qToS);
	var matrix = BitMatrix.createEmpty(loc.dimension, loc.dimension);
	...
	for (var y = 0; y < loc.dimension; y++) {
		for (var x = 0; x < loc.dimension; x++) {
			var xValue = x + 0.5;
			var yValue = y + 0.5;
